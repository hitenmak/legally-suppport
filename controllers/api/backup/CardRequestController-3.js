...
// Helpers
const { d, dd, de, empty, checkValidation, getBool, getStr, getVal, getNum, formatNumber, formatReqFiles, flipOnKey, getBase64FromUrl, formatBytes, capitalizeFirstLetter, isEqual, ex, getDateFormat, ddm, makeList, newObjectId } = require('../../helpers/helpers');
...
exports.details = async (req, res) => {
    ...
    try {
        const resData = DataManipulate.cardRequestDetails(_user._id);
        ...
        cardRequestRecord.save().then(async record => {
            ...
            axios.post(`${process.env.HYPERPAY_ORDER_CREATE_API_URL}`, orderCreatePayload, { 'Content-Type': 'application/json; charset=utf-8' }).then(async (cardOrderRes) => {
                ...
                CardRequest.findOneAndUpdate(
                    { _id: cardRequestRecord._id, 'cardPayment._id': cardOrderId },
                    { $set: { 'cardPayment.$.orderNo': cardOrderData.order_no, } }
                ).exec();
...
exports.updatePaymentStatus = async (req, res) => {
    ...
    try {
        const record = await CardRequest.findOneAndUpdate(
            ...
            // { $set: { 'cardOrder.$.isPaid': true, paymentReceivedAt: new Date(), cardStatus: 1, } },
            { $set: { 'cardOrder.$.isPaid': true, confirmAt: new Date(), cardStatus: 2, } },
            { new: true }
        ).exec();
