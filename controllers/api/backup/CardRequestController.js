...
exports.create = async (req, res) => {
    ...
    try {
        Media.storeCardAttachment('frontDoc,mixDoc,signImg')(req, res, async (err) => {
            ...
            // Check Unique {
            // TODO: uncomment // if (await CardRequest.countDocuments({ userId: _user._id })) return ret.sendFail(Msg.cardRequest.alreadyApplied);
...
exports.createOrder = async (req, res) => {
    ...
    try {
        ...
        cardRequestRecord.save().then(async record => {
            ...
            const message = `amount=${hyperPayConfig.cardOrderAmount}&app_id=${process.env.HYPERPAY_APP_ID}&currency=${hyperPayConfig.cardOrderCurrency}&lang=${language}&merchant_order_id=${merchantOrderId}&return_url=${paymentReturnUrl}&time=${timestamp}&version=${version}`;
            ...
            let orderCreatePayload = {
                'amount': hyperPayConfig.cardOrderAmount,
            ...
            axios.post(`${process.env.HYPERPAY_ORDER_CREATE_API_URL}`, orderCreatePayload, { 'Content-Type': 'application/json; charset=utf-8' }).then(async (cardOrderRes) => {
                ...
                await CardRequest.updateOne({ _id: cardRequestRecord._id }, { $set: { 'cardOrder': { orderNo: cardOrderData.order_no } } }).exec();
                ...
                de(errorThrowKey);
                de(e);
                CardRequest.updateOne({ _id: cardRequestRecord._id }, { $pull: { 'cardOrder': { _id: cardOrderId } } }).exec();
                ret.sendFail(errorMessages[errorThrowKey]);
            });
            // } Create Payment Link
