...
exports.create = async (req, res) => {
    Media.storeSupportAttachment('attachments')(req, res, async (err) => {
        ...
        record.save().then(async updatedRecord => {
            ...
            await supportTicketCreate({toEmail: admin.email, attachments, ticketId: record?.ticketId, requestType: record?.requestType, userName: req.user.name, email: req.user.email, message: latestReply?.message})
...
exports.createticket = async (req, res) => {
    Media.storeSupportAttachment('attachments')(req, res, async (err) => {
        ...
        if(empty(isExistEmail)) { const {hash, salt} = makeHashPassword("12345678");
        let newRecord = {
        ...
        updatedRecord = updatedRecord?.toObject();
         ...
         record.userId = updatedRecord._id;}else{
